...
import { X, Star, UserPlus, Utensils, PhoneCall, Sparkles } from 'lucide-react';
...
export function AddCustomerModal({ isOpen, onClose, onSuccess }: AddCustomerModalProps) {
  ...
  const [activeCallCustomer, setActiveCallCustomer] = useState<{ name: string; phone: string; items?: string } | null>(null);

  if (!isOpen && !isLiveCallOpen) return null;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        if (autoCall) {
          // Open Live AI Voice Call Simulator Modal
          setActiveCallCustomer({ name: customerName, phone: customerPhone, items });
  ...
  return (
    <>
              ...
              <div className="flex items-center justify-between p-5 border-b border-[var(--color-border-subtle)] bg-gray-50/50">
                <div className="flex items-center space-x-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-[var(--color-text-muted)]">Record visit and automatically trigger AI Voice Agent call</p>
                  </div>
                </div>
                <button 
                  onClick={onClose}
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-4 max-h-[80vh] overflow-y-auto">
                ...
                <div className="flex justify-end space-x-3 pt-4 border-t border-[var(--color-border-subtle)]">
                  ...
                  <Button type="submit" disabled={loading} className="flex items-center space-x-2">
                    ...
                    <span>{loading ? 'Saving Customer...' : 'Save & Start AI Call'}</span>
                  </Button>
                </div>
              </form>
            </div>
      ...
      {isLiveCallOpen && activeCallCustomer && (
        <LiveCallModal
          ...
          itemsOrdered={activeCallCustomer.items}
          onCallComplete={onSuccess}
        />
      )}
    </>
  );
